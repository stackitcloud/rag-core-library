...
from src.rag_core_api.mapper.information_piece_mapper import InformationPieceMapper
...
def minimal_text_documents() -> tuple[InformationPiece, Document]:
    ...
    page_content = """STACKIT Object Storage is a flexible scalable cloud storage architecture that stores and manages data as objects.
    ...
    source_doc = InformationPiece(
        ...
        metadata=[KeyValuePair(key=key, value=json.dumps(value)) for key, value in metadata.items()],
    )
...
def test_mapping(minimal_text_documents):
    ...
    mapped_source_doc = InformationPieceMapper.langchain_document2source_document(original_langchain_doc)
    mapped_langchain_doc = InformationPieceMapper.source_document2langchain_document(original_source_doc)

    double_mapped_langchain_doc = InformationPieceMapper.source_document2langchain_document(mapped_source_doc)
    double_mapped_source_doc = InformationPieceMapper.langchain_document2source_document(mapped_langchain_doc)
...
def test_mapping_fails_with_missing_document_url(missing_document_url_documents):
    ...
    with pytest.raises(ValueError, match=DOCUMENT_URL_KEY):
        InformationPieceMapper.source_document2langchain_document(original_source_doc)

    with pytest.raises(ValueError, match=DOCUMENT_URL_KEY):
        InformationPieceMapper.langchain_document2source_document(original_langchain_doc)
