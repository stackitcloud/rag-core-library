...
def generate_text_based_pdf():
    ...
    table2 = Table(table2_data, colWidths=[1*inch, 1.2*inch, 0.9*inch, 0.9*inch, 0.9*inch, 0.9*inch, 1*inch])
    ...
    table3 = Table(table3_data, colWidths=[1*inch, 0.6*inch, 1*inch, 1*inch, 1*inch, 1*inch, 0.8*inch, 0.8*inch])
...
def generate_mixed_content_pdf():
    ...
    # Add annotations
    ax2.annotate('Peak Performance', xy=('Q4', 78), xytext=('Q3', 85),
                arrowprops=dict(arrowstyle='->', color='red', alpha=0.7),
                fontsize=10, ha='center')
    ax2.annotate('Significant Drop', xy=('Q5', 32), xytext=('Q4', 40),
                arrowprops=dict(arrowstyle='->', color='red', alpha=0.7),
                fontsize=10, ha='center')
    ...
    for i in range(len(metrics)):
        for j in range(len(metrics)):
            text = ax3.text(j, i, f'{correlation_data[i, j]:.1f}',
...
def generate_multi_column_pdf():
    ...
    left_frame = Frame(0.75*inch, 0.75*inch, frame_width, frame_height, 
                       leftPadding=6, rightPadding=6, topPadding=6, bottomPadding=6)
    right_frame = Frame(0.75*inch + frame_width + 0.25*inch, 0.75*inch, 
